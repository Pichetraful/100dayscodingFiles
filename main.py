...
# print(birthdays_dict[11, 11]["name"])
...
print(random.choice(letter_templates))
if (today_month,today_day) in birthdays_dict:
    print("Amsterdam is home")
    with open(random.choice(letter_templates), "r") as letter:
        letter_to_send = letter.read()
        a = letter_to_send.replace("[NAME]", birthdays_dict[today_month, today_day]["name"])
        print(a)
    with smtplib.SMTP("smtp.gmail.com", 587) as connection:
        connection.starttls()
        connection.login(user=my_email, password=password)
        connection.sendmail(from_addr=my_email, to_addrs=birthdays_dict[today_month, today_day]["email"], msg=f"Subject: B-day wishes\n\n{a}")
